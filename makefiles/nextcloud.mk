...
nextcloud-mirror: ## Mirror Nextcloud chart + images to DOCR (VERSION=<ver>)
	@$(LAZYOCI) mirror --config $(MIRROR_CONFIG) --chart nextcloud $(if $(VERSION),--version $(VERSION))

.PHONY: nextcloud-images
nextcloud-images: ## List container images in Nextcloud chart
	@$(LAZYOCI) mirror --config $(MIRROR_CONFIG) --chart nextcloud --dry-run --images-only -o json | jq -r '.charts[].versions[].images[].source'
